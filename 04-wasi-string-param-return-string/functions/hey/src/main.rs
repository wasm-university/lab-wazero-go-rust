...
pub unsafe extern "C" fn _print_hello(ptr: u32, len: u32) {
  print_hello(&ptr_to_string(ptr, len));
}
...
/// WebAssembly 1.0.
#[cfg_attr(all(target_arch = "wasm32"), export_name = "return_hello")]
#[no_mangle]
pub unsafe extern "C" fn _return_hello(ptr: u32, len: u32) -> u64 {
    let name = &ptr_to_string(ptr, len);
    let g = return_hello(name);
...
fn return_hello(name: &String) -> String {
